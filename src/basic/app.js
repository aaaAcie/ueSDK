...
const webRtcPlayer = require('./wRtc')
...
//自动全屏
var matchViewportResolution = false;
...
function showConnectOverlay() {
	...
	startText.innerHTML = '点击播放';
	
	
	// startText.style.backgroundImage ="url('/images/nn.jpg')";
	// startText.style.backgroundSize="100%";
	// //startText.style.backgroundRepeat="no-repeat";
	// startText.style.width="100%";
	// startText.style.height="100%";
	// startText.style.display="table";
	...
	// div.id = "playText";
	// div.className = "playTextName";
	// div.innerHTML = '点击播放';
	// //div.style.padding = "50%";
	// //div.style.lineHeight = "1080px";
	// div.style.display="table-cell";
	// div.style.verticalAlign="middle";
	// startText.appendChild(div);
	
	// //startText.style.textAlign="center";
	// //startText.style.verticalAlign="middle";
	// //startText.style.position = "absolute";
	// //startText.style.top="50%";
	// //startText.style.left="50%";
	// //startText.style.zIndex=40;
	// //startText.style.backgroundSize="1920px 1080px";
...
var inputOptions = {
	...
	c: ControlSchemeType.HoveringMouse,
...
function onQuit_Click() {
    ...
    // var cmdStr = "taskkill /f /t /im 360se.exe";
    // cmd.run(cmdStr,0);
    // cmd = null;
    // alert("关闭成功！！！");
}
...
function start() {
	...
	if (!connect_on_load || is_reconnection) {
		...
		if(doonce)
		{
			setTimeout(function () {
			document.getElementById('videoPlayOverlay').click();
			//document.getElementByClassName('clickableState').click();
			}, 1000 );
			doonce = false;
			//alert("123");
		}
...
function connect() {
	...
	// console.log(window.location.href,window.location.href.replace('http://', 'ws://'))
	...
	ws.onmessage = function (event) {
		...
		if (msg.type === 'config') {
			...
			console.log(`playersCountttt: ${msg.count}`);			
...
function load() {
	...
	//onExpandOverlay_Click(); 不开启overlaySettings
	setupHtmlEvents();
