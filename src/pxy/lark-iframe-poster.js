  ...
  var iframePoster = function (iframeElement, config) {
    ...
    if (config.listenKeyboard) {
      document.addEventListener("keydown", function (e) {
        sendToIframe(
          EventTypes.LK_IFRAME_POSTER_OPERATE_KEY_DOWN,
          {
            key: e.code,
            isRepeat: e.repeat,
          },
      ...
      document.addEventListener("keyup", function (e) {
        sendToIframe(
          EventTypes.LK_IFRAME_POSTER_OPERATE_KEY_UP,
          {
            key: e.code,
          },
          ""
    ...
    function sendWheelUp(x, y) {
      ...
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_OPERATE_MOUSE_WHEEL,
        {
          x: x,
          y: y,
          wheel: 120,
    ...
    function sendMouseMove(x, y, rx, ry) {
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_OPERATE_MOUSE_MOVE,
        {
          x: x,
          y: y,
          rx: rx,
    ...
    function sendMouseDown(button, x, y) {
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_OPERATE_MOUSE_DOWN,
        {
          button: button,
          x: x,
          y: y,
    ...
    function sendMouseUp(button, x, y) {
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_OPERATE_MOUSE_UP,
        {
          button: button,
          x: x,
          y: y,
    ...
    function setShowMobileControlBall(show) {
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_UI_MOBILE_CONTROL_BALL,
        show,
        ""
      );
    }
    ...
    function setShowMobileKeyboard(show) {
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_UI_MOBILE_VIRTUAL_KEYBOARD,
        show,
        ""
      );
    }
    ...
    function setShowMobileVritualMouse(show) {
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_UI_MOBILE_VIRTUAL_MOUSE,
        show,
        ""
      );
    }
    ...
    function setMobileForcelandscape(force) {
      sendToIframe(
        EventTypes.LK_IFRAME_POSTER_UI_MOBILE_FORCE_LANDSCAPE,
        force,
        ""
      );
    }
    ...
    function sendBinaryToRenderServerAppDataChannel(binary) {
      sendToIframe(
        EventTypes.LK_DATA_CHANNEL_RENDERSERVER_SEND_BINARY,
        binary,
        ""
      );
    }
    ...
    poster.sendTextToRenderSererAppDataChannel =
      sendTextToRenderSererAppDataChannel;
    poster.sendBinaryToRenderServerAppDataChannel =
      sendBinaryToRenderServerAppDataChannel;
    return poster;
  };
