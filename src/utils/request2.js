...
const request = axios.create({
  // timeout: 5000
  baseURL: 'https://dijd.xfdev.cn/openApi/'
...
request.interceptors.request.use(config => {
  ...
  config.headers["X-Auth-Version"]= '1.0'
  config.headers["X-Auth-Nonce"] = config.url
  config.headers["X-Auth-Timestamp"] = config.url
  config.headers["X-Auth-Appid"] = 'r8x3N12260E693Cm'
  config.headers["X-Auth-Signature"] = config.url
