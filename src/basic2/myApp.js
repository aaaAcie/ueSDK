...
//   autoPlayAudio = typeof config.autoPlayAudio !== 'undefined' ? config.autoPlayAudio : true;
  ...
  webRtcPlayerObj.onDataChannelMessage = function(data) {
      ...
      if (view[0] === ToClientMessageType.QualityControlOwnership) {
          ...
          console.log('invalidateFreezeFrameOverlay');
...
function setupStats(){
  ...
  webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
    ...
    // let statsObj = {}
    statsObj['Duration'] = `${timeFormat.format(runTimeHours)}:${timeFormat.format(runTimeMinutes)}:${timeFormat.format(runTimeSeconds)}`
    statsObj['Video Resolution'] = `${
    ...
    statsObj['Received'] = `${receivedBytesMeasurement}): ${numberFormat.format(receivedBytes)}`
    statsObj['Frames Decoded'] = `${aggregatedStats.hasOwnProperty('framesDecoded') ? numberFormat.format(aggregatedStats.framesDecoded) : 'Chrome only'}`
    statsObj['Packets Lost'] = `${aggregatedStats.hasOwnProperty('packetsLost') ? numberFormat.format(aggregatedStats.packetsLost) : 'Chrome only'}`
    statsObj['Framerate'] = `${aggregatedStats.hasOwnProperty('framerate') ? numberFormat.format(aggregatedStats.framerate) : 'Chrome only'}`
    statsObj['Frames dropped'] = `${aggregatedStats.hasOwnProperty('framesDropped') ? numberFormat.format(aggregatedStats.framesDropped) : 'Chrome only'}`
    statsObj['Net RTT (ms)'] = `${aggregatedStats.hasOwnProperty('currentRoundTripTime') ? numberFormat.format(aggregatedStats.currentRoundTripTime * 1000) : 'Can\'t calculate'}`
    statsObj['Browser receive to composite (ms)'] = `${aggregatedStats.hasOwnProperty('receiveToCompositeMs') ? numberFormat.format(aggregatedStats.receiveToCompositeMs) : 'Chrome only'}`
    statsObj['Bitrate (kbps)'] = `${aggregatedStats.hasOwnProperty('bitrate') ? numberFormat.format(aggregatedStats.bitrate) : 'Chrome only'}`
    statsObj['Video Quantization Parameter'] = `${VideoEncoderQP}`
  ...
  webRtcPlayerObj.latencyTestTimings.OnAllLatencyTimingsReady = function(timings) {
    ...
    console.log('latencyStatsInnerHTML');
    console.log(latencyStatsInnerHTML)
  }
}
function passStats() {
...
async function load(WSurl,domId) {
  ...
  // start(WSurl);
  let playerElement = await connect(WSurl)
  return new Promise((resolve,reject) => {
    if(playerElement){
        resolve(ws)
